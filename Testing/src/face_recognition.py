...
def crop_resize_face(img, xyxy):

    x1 = np.maximum(int(xyxy[0]) - 16, 0)
    y1 = np.maximum(int(xyxy[1]) - 16, 0)
    x2 = np.minimum(int(xyxy[2]) + 16, img.shape[1])
    y2 = np.minimum(int(xyxy[3]) + 16, img.shape[0])
    img = img.copy()
    cropped_img = img[y1:y2,x1:x2]
    resized_img = cv2.resize(cropped_img, (160, 160), interpolation=cv2.INTER_CUBIC)
    return resized_img
...
def detect(
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            # Get the SVC Model ready
            ## Load the model
            print('Loading feature extraction model')
            facenet.load_model(facenet_model_path)
            with open(svc_model_path, 'rb') as infile:
                (classifymodel, class_names) = pickle.load(infile)
            print('Loaded classifier model from file "%s"' % svc_model_path)
            ...
            # Get the face detect model ready 
            ## Load model
            img_size = 640
            conf_thres = 0.6
            iou_thres = 0.5
                ...
                for i, det in enumerate(pred):  # detections per image
                    ...
                    if len(det):
                        ...
                        for j in range(det.size()[0]):
                            ...
                            #im0 = crop_resize_face(im0, xyxy)

                            scaled = crop_resize_face(im0, xyxy)
                            img = load_image(np.array(scaled), False, False, 160)
